...
// Startup.cs
public class Startup : IAsyncStartupScript
{
...
public class WeatherTestInitializer : IAsyncDITestInitializer
{
...
In this example, the `WeatherTestInitializer` class configures the DI container for the test project by reusing the same `Startup` class used by the main application. The `ConfigureAsync` method configures the DI container by invoking the `ConfigureServices` method of the `Startup` class, ensuring that the same services are registered in both the main application and the test project. The `InitializeAsync` method initializes any services needed for the tests, such as loading test data into the database. Both methods are asynchronous to allow interaction with async infrastructure during test setup.
...
For testing components that require DI, such as manager classes, controllers, or other DI services, the `ComponentTest<TComponent, TInitializer>` class should be used. This class provides additional utilities for instantiating the component under test and managing the DI container. The `TComponent` type parameter specifies the type of the component being tested, while the `TInitializer` type parameter specifies the `IAsyncDITestInitializer` implementation to use for configuring the DI container, as detailed in the [Setup](#setup) section.
...
public class WeatherTestInitializer : IAsyncDITestInitializer
{
    ...
    public static async ValueTask InitializeAsync(IServiceProvider serviceProvider, CancellationToken cancellationToken)
    {
        ...
        // load test data
        await serviceProvider.InitializeTestDatabaseAsync<WeatherTestDatabaseLoader>(cancellationToken);
    }
}
```

The `MockDatabaseTransactions<TDbContext>` extension method configures the `Wkg.AspNetCore.Transactions` framework to roll back all `ITransaction<TDbContext>` instances created during the test, regardless of their outcome, allowing you to write your service code as usual without having to add additional logic to disable transactions when your components are being tested. Read more about transactions in the [Transactions](#transactions-namespace) section.

The `InitializeTestDatabaseAsync<TAsyncTestDatabaseLoader>` extension method initializes the database with test data using the specified `IAsyncTestDatabaseLoader` implementation. This allows you to load test data into the database before running your tests, ensuring that the database is in a known state before the tests are executed. The test database itself may be automatically created and migrated to the latest schema version using external infrastructure, such as Docker containers and GitLab CI/CD pipeline stages.
...
// UserManagerTests.cs
[TestClass]
public class UserManagerTests : ComponentTest<UserManager, WeatherTestInitializer>
{
    [TestMethod]
    public Task TestCreateUser() => UsingComponentAsync(async (userManager, cancellationToken) =>
    {
        // arrange
...
To load test data into the database before running any tests, you can implement a custom test database loader by inheriting from the `AsyncTestDatabaseLoader<TSelf, TDbContext>` class and implementing the `IAsyncTestDatabaseLoader<TDbContext>` interface.

```csharp
// WeatherTestDatabaseLoader.cs
class WeatherTestDatabaseLoader : AsyncTestDatabaseLoader<WeatherTestDatabaseLoader, WeatherDbContext>, IAsyncTestDatabaseLoader<WeatherDbContext>
{
    public async ValueTask InitializeDatabaseAsync(WeatherDbContext dbContext, IServiceProvider serviceProvider, CancellationToken cancellationToken)
    {
...
{
    [TestMethod]
    public Task TestSoftDelete() => UsingDbContextAsync(async (dbContext, cancellationToken) =>
    {
...
For scenarios where you want to exercise code inside a real transaction scope while still isolating the database state per test, `Wkg.AspNetCore.TestAdapters` provides a set of transactional test base classes. These classes integrate with the `ITransaction<TDbContext>` infrastructure from `Wkg.AspNetCore.Transactions` and the DI scoping behavior of `DIAwareTestBase<TInitializer>`.

At the lowest level, `TransactionalTest<TDbContext, TInitializer>` lets you run arbitrary code against a transactional `DbContext` and the scoped `IServiceProvider`:

```csharp
// WeatherTransactionalTests.cs
[TestClass]
public class WeatherTransactionalTests : TransactionalTest<WeatherDbContext, WeatherTestInitializer>
{
    [TestMethod]
    public Task TestTransactionalWork() => UsingTransactionAsync(async (dbContext, serviceProvider, cancellationToken) =>
    {
        // arrange
        ...
        User? createdUser = await dbContext.Users
            .FirstOrDefaultAsync(u => u.Email == "johndoe@example.com", cancellationToken);
...
In this example, the `UsingTransactionAsync` helper opens a read-only transactional scope for the outermost DI scope created by `DIAwareTestBase<TInitializer>`. All database operations performed through `WeatherDbContext` and any services that use `ITransaction<WeatherDbContext>` participate in the same ambient transaction. Because `MockDatabaseTransactions<WeatherDbContext>` is configured in the test initializer, the transaction is rolled back once the test completes.

For component-level tests that should run inside a transaction, you can use `TransactionalComponentTest<TComponent, TDbContext, TInitializer>`. It combines the DI capabilities of `ComponentTest` with the transactional helpers of `TransactionalTest`:

```csharp
// UserManagerTransactionalTests.cs
[TestClass]
public class UserManagerTransactionalTests : TransactionalComponentTest<UserManager, WeatherDbContext, WeatherTestInitializer>
{
    [TestMethod]
    public Task TestCreateUserTransactional() => UsingComponentAsync(async (userManager, serviceProvider, dbContext, cancellationToken) =>
    {
        // arrange
        ...
        Assert.IsTrue(result.IsSuccess);
        User? createdUser = await dbContext.Users
            .FirstOrDefaultAsync(u => u.Email == "alice@example.com", cancellationToken);
        Assert.IsNotNull(createdUser);
    });
}
```

Here, `UsingComponentAsync` activates the `UserManager` using the scoped service provider, initializes it inside the ambient transaction, and then passes both the manager, the scoped `IServiceProvider`, and the transactional `WeatherDbContext` to the test delegate. As with `TransactionalTest`, all database changes are rolled back at the end of the test when `MockDatabaseTransactions<WeatherDbContext>` is enabled.

For ASP.NET Core controllers that should be tested inside a transaction with model validation support, `TransactionalControllerTest<TController, TDbContext, TInitializer>` adds additional plumbing for `HttpContext` and model validation:

```csharp
// UserControllerTransactionalTests.cs
[TestClass]
public class UserControllerTransactionalTests : TransactionalControllerTest<UserController, WeatherDbContext, WeatherTestInitializer>
{
    [TestMethod]
    public Task TestCreateUserEndpoint() => UsingComponentAsync(new CreateUserRequest
    {
        Name = "Bob",
        Email = "bob@example.com"
    }, async (controller, serviceProvider, dbContext, cancellationToken) =>
    {
        // act
        IActionResult result = await controller.CreateUserAsync(cancellationToken);

        // assert
        Assert.IsInstanceOfType(result, typeof(OkResult));
        User? createdUser = await dbContext.Users
            .FirstOrDefaultAsync(u => u.Email == "bob@example.com", cancellationToken);
        Assert.IsNotNull(createdUser);
    });
}
```

`TransactionalControllerTest` configures a `HttpContext` for the controller, validates the supplied request model using `TryValidateModel`, and then executes the test delegate inside the same ambient transaction as other transactional tests. This pattern is useful for end-to-end style tests of controller actions that interact with the database while still ensuring that all changes are rolled back after each test.